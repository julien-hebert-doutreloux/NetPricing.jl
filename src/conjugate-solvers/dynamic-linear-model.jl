...
# Set demands
function set_demands(cmodel::ConjugateDynamicLinearModel, demands, discount::AbstractDict{Int,<:Real})
    a1 = tolled_arcs(cmodel.prob)
    cmodel.demands = Dict(a => get(demands, a, 0.) * get(discount, a, 1.) for a in a1)
    return nothing
end
